...
export default function MaterialOutbound() {
  ...
  const handleSave = async () => {
    try { const values = await form.validateFields(); setSaving(true);
      await api.post('/work-orders/pick-material', values);
      message.success('生产领料出库单已创建');
  ...
  return (
    <div>
      <h3 style={{ marginBottom: 16 }}>生产领料 — 车间按工单领料出库</h3>
      <Button type="primary" icon={<PlusOutlined />} style={{ marginBottom: 16 }} onClick={() => { form.resetFields(); setModalOpen(true); }}>新建领料单</Button>
      <Table columns={columns} dataSource={data} rowKey="work_order_id" loading={loading} pagination={{ pageSize: 20 }} />
      <Modal title="新建生产领料单" open={modalOpen} onOk={handleSave} confirmLoading={saving} onCancel={() => { setModalOpen(false); form.resetFields(); }}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="raw_material_id" label="领料物料" rules={[{ required: true }]}>
          ...
          <Form.Item name="pick_quantity" label="领料数量" rules={[{ required: true }]}><InputNumber min={1} style={{ width: '100%' }} /></Form.Item>
          <Form.Item name="workshop_id" label="领料车间" rules={[{ required: true }]}><Input placeholder="例: WS001" /></Form.Item>
          <Form.Item name="applicant" label="申请人" rules={[{ required: true }]}><Input placeholder="领料人姓名" /></Form.Item>
