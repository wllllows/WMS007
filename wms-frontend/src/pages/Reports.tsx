...
export default function Reports() {
  ...
  const tabItems = [
    ...
    { key:'work', label:`工单(${workOrders.length})`, children: <Table columns={[{title:'工单号',dataIndex:'work_order_id',width:120},{title:'类型',dataIndex:'work_order_type'},{title:'状态',dataIndex:'status',render:(v:string)=><Tag color={v==='completed'?'green':'blue'}>{v}</Tag>},{title:'开工',dataIndex:'start_time',render:(v:string)=>new Date(v).toLocaleDateString()},{title:'下达日期',dataIndex:'issue_date'}]} dataSource={workOrders.map((w:any,i:number)=>({...w,key:i}))} pagination={{pageSize:15}} size="small"/> },
    { key:'outsource', label:`外协(${outsourcingOrders.length})`, children: <Table columns={[{title:'订单号',dataIndex:'outsourcing_order_id',width:140},{title:'状态',dataIndex:'order_status',render:(v:string)=>{const cm:Record<string,string>={completed:'green',processing:'blue',pending:'orange',settled:'purple'};return <Tag color={cm[v]||'default'}>{v}</Tag>;}},{title:'金额',dataIndex:'order_amount',render:(v:number)=>`¥${v}`},{title:'日期',dataIndex:'order_date'},{title:'厂商',dataIndex:'supplier_id'}]} dataSource={outsourcingOrders.map((o:any,i:number)=>({...o,key:i}))} pagination={{pageSize:15}} size="small"/> },
  ];
